...
export class MetricsRegistry {
	...
	private create(type: 'counter', name: string, help?: string): CounterType {
		...
		return new Proxy(counter, {
			get(target, prop) {
				if (prop === 'collect' || prop === 'get' || prop === 'inc' || prop === 'reset') {
					return function (labels?: Labels) {
						const mergedLabels = { ...defaultLabels, ...labels };
						return Reflect.get(target, prop)(mergedLabels);
					};
				}
				if (prop === 'add' || prop === 'set') {
					return function (value: number, labels?: Labels) {
						const mergedLabels = { ...defaultLabels, ...labels };
						return Reflect.get(target, prop)(value, mergedLabels);
					};
				}
